...
from services.gemini_client import create_gemini_model, invoke_with_retry
import os
import json

api_key = os.getenv("RESUME_API")
model = create_gemini_model(model_name="gemini-2.5-flash", temperature=1.0, api_key=api_key)
...
"""
        response = invoke_with_retry(structured_model, [HumanMessage(content=prompt)])
...
"""
            forced_response = invoke_with_retry(model, [HumanMessage(content=forced_prompt)])
